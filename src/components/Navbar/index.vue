...
const { navigationBarBackgroundColor, navigationBarTitleText, navigationBarTextStyle } =
  useGlobalStyle()
...
<template>
  <view class="head-wrapper">
    <view :class="['page-head', 'head-fixed', 'shadow']">
      <!-- 顶部状态栏 -->
      <view class="status-bar"></view>
      <!-- navbar -->
      <view :class="['navbar-wrapper', 'flex', 'flex-nowrap', 'justify-between', 'items-center']">
        <view class="flex flex-nowrap items-center h-full w3/10 min-w3/10">
          <slot name="left">
        ...
        <view
          class="navbar__center flex flex-nowrap justify-center items-center h-full w2/5 min-w2/5"
        >
          <slot>
            <text>{{ navbarTitle }}</text>
          </slot>
        </view>
        <view class="flex flex-nowrap justify-end items-center h-full w3/10 min-w3/10">
